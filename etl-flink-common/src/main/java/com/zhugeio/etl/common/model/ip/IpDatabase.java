...
public class IpDatabase implements Serializable {
    ...
    private void initialize() {
        ...
        LOG.debug("IP数据库初始化完成: offset={}, indexSize={}", offset, index.length);
    }
    ...
    public String[] find(String ip) {
        ...
        try {
            ...
            LOG.error("IP查询异常: {}", ip, e);
            return new String[]{"", "", ""};
        }
    }

    /**
     * 查找IP地址 (InetAddress, 支持IPv6)
     */
    public String[] find(InetAddress inetAddress) {
        try {
            if (inetAddress instanceof Inet4Address) {
                return find(inetAddress.getHostAddress());
            } else if (inetAddress instanceof Inet6Address) {
                // IPv6支持 (需要使用AWReader)
                return findIPv6(inetAddress);
            } else {
                return new String[]{"", "", ""};
            }
        } catch (Exception e) {
            LOG.error("IP查询异常: {}", inetAddress, e);
            return new String[]{"", "", ""};
        }
    }

    /**
     * IPv6查询 (简化版,需要完整实现AWReader逻辑)
     */
    private String[] findIPv6(InetAddress ipv6) {
        // TODO: 实现IPv6查询逻辑
        LOG.warn("IPv6查询暂未实现: {}", ipv6);
        return new String[]{"", "", ""};
    }
